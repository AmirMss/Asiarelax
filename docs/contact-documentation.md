...
Contient le **sélecteur de langue FR/EN** (`.lang-switcher`) ajouté après le `<nav>`, avec un lien vers `en/contact.html`.
...
> **Mise à jour (05/03/2026) :** Le texte horaires est passé de `Lun-Ven` / `Sam` à **`Lundi - Vendredi`** / **`Samedi`** (sur deux lignes séparées avec `<br>`), pour plus de lisibilité.
...
Un widget reCAPTCHA Google est intégré avant le bouton d'envoi :
...
La validation JS vérifie `grecaptcha.getResponse()` : si vide, l'envoi est bloqué avec un message d'erreur dans `#captcha-error`.

> ⚠️ **La clé sitekey actuelle est la clé de test publique Google** — elle accepte tous les robots. Remplacer par une clé de production liée à `asiarelaxation.fr`.
...
*Documentation générée le 05/03/2026 — Version 1.2*
*Mise à jour 05/03/2026 : ajout sélecteur langue FR/EN, horaires reformatés, section reCAPTCHA*
